...
export default async function (req, res) {
    try {
        ...
        const rings = await Ringtone.find({ downloads: { "$in": [user.id] } }).populate({ path: 'uid', model: User, select: 'username' }).select('ringID title thumbnail likes origin downloads createdAt')
        if (!rings) {
